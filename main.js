...
module.exports.loop = function () {
    ...
    if (
        harvesters.length <= 2 &&
        dropminers.length == 0 &&
        !_.isEmpty(spawn.room.memory.creepBuildQueue.queue)
    ) {
        const job = spawn.room.memory.creepBuildQueue.queue[0];

        if (
            job.name != role.HARVESTER ||
    ...
    if (
        spawn.spawning === null &&
        spawn.room.memory.creepBuildQueue.queue.length == 0
    ) {
        if (
            (harvesters.length === 0 && dropminers.length === 0) ||
            (dropminers.length > 0 &&
                (couriers.length === 0 || gophers.length === 0)) ||
            (structures.storage && linkBaseHarvesters.length === 0)
        ) {
            energyCapacityAvailable = spawn.room.energyAvailable;
            console.log(
                "⚠️ INFO: Limited energy available, downsizing build allowence to",
